...
try {
  ...
  if (platform() !== "win32") {
    ...
    const unsafeGeminiDir = join(root, ".gemini-unsafe");
    await mkdir(unsafeGeminiDir);
    await symlink(join(outsideRoot, "secret.txt"), join(unsafeGeminiDir, "GEMINI.md"));
    const unsafeGeminiWorkspace = await registry.openWorkspace({
      path: root,
