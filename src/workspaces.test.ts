...
test("a checkout exposes initial and nested instruction context while filtering outside symlinks", async (t) => {
  ...
  const geminiAgentDir = join(context.root, ".gemini");
  await mkdir(geminiAgentDir);
  await writeFile(join(geminiAgentDir, "GEMINI.md"), "gemini global instructions\n");
  const geminiWorkspace = await context.registry.openWorkspace({
    path: context.root,
    agentGlobalDir: geminiAgentDir,
  });
  assert.deepEqual(
    geminiWorkspace.agentsFiles.map((file) => file.content),
  ...
  if (platform() !== "win32") {
    ...
    const unsafeConfig = loadConfig({
      ...
      DEVSPACE_OAUTH_OWNER_TOKEN: TEST_OWNER_TOKEN,
      PORT: "1",
    });
    ...
    const unsafeGeminiDir = join(context.root, ".gemini-unsafe");
    await mkdir(unsafeGeminiDir);
    await symlink(
      join(context.outsideRoot, "secret.txt"),
      join(unsafeGeminiDir, "GEMINI.md"),
    );
    const unsafeGeminiWorkspace = await context.registry.openWorkspace({
      path: context.root,
      agentGlobalDir: unsafeGeminiDir,
    });
    assert.deepEqual(
      unsafeGeminiWorkspace.agentsFiles.map((file) => file.content),
      ["root instructions\n"],
    );
  }
});
