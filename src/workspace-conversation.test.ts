...
test("conversation reuse preserves caller-selected global instructions", async (t) => {
  const { root, project, registry } = await fixture(t);
  const agentGlobalDir = join(root, ".gemini");
  await mkdir(agentGlobalDir);
  await writeFile(join(agentGlobalDir, "GEMINI.md"), "gemini global instructions\n");
  const input = { path: project, agentGlobalDir };

  const first = await registry.openWorkspace(input, { conversationScopeId: "chat-1" });
  const second = await registry.openWorkspace(input, { conversationScopeId: "chat-1" });
